...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train', embedding_type='cnn-rnn',
        ...
        self.filenames = self.load_filenames(split_dir)# pictures files
        self.embeddings = self.load_embedding(split_dir, embedding_type)# the text embedding of each picture
        self.class_id = self.load_class_id(split_dir, len(self.filenames))# the class id of each picture
    ...
    def load_all_captions(self):
        ...
        if cfg.DATASET_NAME=='flowers':
            for key in self.filenames:
                class_file_name = "class_" + str(self.class_id[class_i]).zfill(5)
                class_i = class_i + 1
                caption_name = '%s/text/%s/%s.txt' % (self.data_dir, class_file_name, key)
