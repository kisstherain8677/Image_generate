...
class condGANTrainer(object):
        ...
        if cfg.TRAIN.NET_G == '':
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                for i in range(embedding_dim):
                    print(t_embeddings[:, i, :].shape)
                    # t_embeddings[:, i, :]——batchsize*1024 对应24个种类，dim是每个种类的一句描述。1024是每个描述的 embedding
                    tmp=t_embeddings[:, i, :]
                    fake_imgs, _, _ = netG(noise, t_embeddings[:, i, :])#[1,1024]
                    if cfg.TEST.B_EXAMPLE:
    ...
    def gen_example(self, cap_path):
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        # convert text to embedding by char-cnn-rnn model
        print(cap_path)
        ...
        modelType = 'icml'
        net_state_dict = extract_char_cnn_rnn_weights(cfg.TEXT_ENCODER_PATH,
                                                      cfg.DATASET_NAME, modelType)
        net = ccr.char_cnn_rnn(cfg.DATASET_NAME, modelType)
        ...
        #generate pic by this embedding
        if cfg.TRAIN.NET_G == '':
            print('Error: the path for morels is not found!')
        else:
            netG = G_NET()
            ...
            nz = cfg.GAN.Z_DIM
            noise = Variable(torch.FloatTensor(self.batch_size, nz))
            noise.data.resize_(self.batch_size, nz)
            noise.data.normal_(0, 1)
            if cfg.CUDA:
                netG.cuda()
                noise = noise.cuda()
